> 本文保留原 Zotero-arXiv-Daily 邮件工作流的使用说明。Zotero 插件版已更名为
> [Zotero Paper Radar（论文雷达）](./README.md)，安装插件请从新首页开始。
> 下文的上游项目链接、署名和赞助信息属于原项目。
...
_Zotero-arXiv-Daily_ finds arxiv papers that may attract you based on the context of your Zotero library, and then sends the result to your mailbox📮. It can be deployed as Github Action Workflow with **zero cost**, **no installation**, and **few configuration** of Github Action environment variables for daily **automatic** delivery.
...
本仓库同时保留原有 GitHub Action 邮件流程，并提供新的 Zotero 9 插件版。插件版把画像、推荐、反馈、论文入库和任务中心放回 Zotero，由用户手动点击运行，不需要 Python、SMTP、GitHub Actions 或常驻云端服务。

- 从 Zotero 库构建可编辑的近期兴趣和长期兴趣画像。
- 每次只基于画像匹配 arXiv 候选，不重复对全库做推荐排序。
- 推荐数量、画像、时间范围和 CV/LLM 分类可在工作台调整。
- 卡片展示六项可解释分数、命中画像、代表论文、关键词、摘要来源和模型指纹。
- Embedding 必需且可配置为云端或本地 OpenAI-compatible 服务；LLM 可选，关闭时保留 arXiv 原始摘要。
- 支持反馈学习、幂等保存并归类、PDF 独立重试、检查点恢复和脱敏诊断。

插件安装、模型配置、隐私边界和排障见
[`zotero-plugin/docs`](./zotero-plugin/docs/installation.md)。维护者构建 XPI 使用
`.github/workflows/plugin-build.yml`；终端用户不需要配置该 Action。
...
| ARXIV_QUERY     |    ✅    | str  | The categories of target arxiv papers. Use `+` to concatenate multiple categories. The default focuses on computer vision and LLM/NLP papers. Find the abbr of your research area from [here](https://arxiv.org/category_taxonomy).                                                                  | cs.CV+cs.CL                   |
...
| MAX_PAPER_NUM   |          | int  | The maximum number of the papers presented in the email. This value directly affects the execution time of this workflow, because it takes about 70s to generate TL;DR for one paper. `-1` means to present all the papers retrieved.                                                                | 5                             |
...
> [!TIP]
> `ARXIV_QUERY`, `MAX_PAPER_NUM`, `SMTP_SERVER`, and `SMTP_PORT` can be configured as either Secrets or Repository Variables. Repository Variables take precedence for non-sensitive settings because they are easier to edit.
...
| INTEREST_PROFILE_CACHE               |          | bool  | Cache generated Zotero interest profiles between Action runs.                                                                                                                                                                                                                                   | 1                                       |
| INTEREST_PROFILE_MAX                 |          | int   | Maximum number of interest profiles built from the Zotero library.                                                                                                                                                                                                                              | 10                                      |
| INTEREST_PROFILE_REPRESENTATIVES     |          | int   | Representative Zotero papers shown for each profile and used for explanation.                                                                                                                                                                                                                   | 3                                       |
| INTEREST_PROFILE_CLUSTER_THRESHOLD   |          | float | Similarity threshold for grouping Zotero papers into the same interest profile. Higher values create narrower profiles.                                                                                                                                                                         | 0.72                                    |
| INTEREST_PROFILE_MAX_MEMBERS         |          | int   | Soft upper bound for papers grouped into one profile. Oversized profiles are split to keep explanations specific.                                                                                                                                                                               | 45                                      |
| INTEREST_PROFILE_DISPLAY_MIN_MEMBERS |          | int   | Minimum profile size shown in email/log summaries. This hides isolated one-paper profiles from the main explanation.                                                                                                                                                                            | 2                                       |
| INTEREST_PROFILE_SCORING_MIN_MEMBERS |          | int   | Minimum profile size used for recommendation scoring, with fallback when all profiles are smaller.                                                                                                                                                                                              | 2                                       |
| INTEREST_PROFILE_DIVERSITY_PENALTY   |          | float | Small score penalty for repeated matches from the same profile, improving topic diversity in the final email.                                                                                                                                                                                   | 0.35                                    |
| RECENT_INTEREST_DAYS                 |          | int   | Zotero papers added within this many days are used to build recent-interest profiles. Set `0` to disable recent-interest scoring.                                                                                                                                                               | 90                                      |
| RECENT_INTEREST_WEIGHT               |          | float | Weight of recent-interest profiles in the final recommendation score.                                                                                                                                                                                                                           | 0.6                                     |
| LONG_TERM_INTEREST_WEIGHT            |          | float | Weight of long-term profiles in the final recommendation score.                                                                                                                                                                                                                                 | 0.4                                     |
...
_Zotero-arXiv-Daily_ firstly retrieves all the papers in your Zotero library and all the papers released in the previous day, via corresponding API. Then it calculates the embedding of each paper's abstract via an embedding model. The score of a paper is its weighted average similarity over all your Zotero papers (newer paper added to the library has higher weight).

The recommender also builds recent and long-term interest profiles from your Zotero library. Recent profiles use papers added within `RECENT_INTEREST_DAYS`, while long-term profiles use the full library. Each profile groups semantically similar Zotero papers, splits overly broad groups, extracts profile-specific domain phrases with TF-IDF-style scoring, filters generic words, and keeps representative papers. The final score combines recent and long-term matches using `RECENT_INTEREST_WEIGHT` and `LONG_TERM_INTEREST_WEIGHT`. The email shows both recent and long-term profile sections and, for each recommended arXiv paper, explains which profile it matched, how cohesive that profile is, which recent/long-term profiles contributed, and which Zotero papers were most similar. A small diversity penalty prevents one profile from occupying all recommended slots.
